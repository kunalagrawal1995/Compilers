...
int BIN_OP::calculate_label(int parent, int is_left){
	...
	operand1->calculate_label(-1, -1); 
	operand2->calculate_label(-1, -1);

	label = max(max(operand1->label, 1 + operand2->label), 2);
	f_label = max(max(operand1->f_label, 1 + operand2->f_label), 2);
}
...
void BIN_OP::generate_code(int n){
	...
	if(expr_type != "float" && operand_type != "FLOAT"){ // anything other than float works like normal integers
		if(operand1->only_int == false && operand2->only_int == false){
			manage_expr(operand1, operand2, 1, 1);
...
int UN_OP::calculate_label(int parent, int is_left){
	operand1->calculate_label(-1, -1);
	label = operand1->label + 1;
	f_label = operand1->f_label + 1;
	return label;
}
...
int EXP_ASSIGN::calculate_label(int parent, int is_left){
	exp1->calculate_label(-1, 0);
	exp2->calculate_label(-1, 1);
	label = max(max(exp1->label, exp2->label + 1), 3);
	f_label = max(exp1->f_label, exp2->f_label + 1);
}
...
void EXP_ASSIGN::generate_code(int n){
	...
	// struct assignment, need to copy the entire data to new place
		...
		manage_expr(exp1, exp2, 0, 1); // do not deref the first exp
		int reg = register_stack.top();
...
int FUNC::calculate_label(int parent, int is_left){
	...
	for (int i = 0; i < param.size(); ++i){
		param[i]->calculate_label(-1, -1);
		label = max(label, param[i]->label);
		f_label = max(f_label, param[i]->f_label);
	}

	int fn_size = 1;

	if(fnname != "printf")
		fn_size = (globalSymTab->sym_rows).find(fnname)->second->size;

	if(fn_size > 1){
		label = max(label, 1);
		f_label = max(f_label, 1);
	}
}
...
int IDENTIFIER_REF::calculate_label(int parent, int is_left){
	// label = is_left;
	// in mips no adding from the register
	label = 1; 
	f_label = 1;
	return label;
...
int ARRAYREF::calculate_label(int parent, int is_left){	
	s->calculate_label(-1, -1); 
	exp->calculate_label(-1, -1);
	label = max(s->label, 1 + exp->label);
	f_label = max(s->f_label, 1 + exp->f_label);
...
int ARROW::calculate_label(int parent, int is_left){
	ref->calculate_label(-1, -1);
	label = max(ref->label, 1);
	f_label = max(ref->f_label, 1);
}
...
void swap_register_stack(int f){
	...
	if(f == 1){
		...
		float_register_stack.pop();
		float_register_stack.push(freg);
		float_register_stack.push(freg2);
	}
}
...
void manage_expr(ExpAst *expr1, ExpAst *expr2, int e1, int e2){
	// if(expr1->only_int == false && expr2->only_int == false){	
		...
		int reg = register_stack.top();
		int freg = float_register_stack.top(); 
		expr1->generate_code(e1);
		string code_s;
		if(expr2->label >= max_stack_size || expr2->f_label >= max_stack_size){
			code_s = "addi $sp, $sp, -4";
			cout << code_s << endl;
			code_s = "sw $" + to_string(reg) + ", 0($sp)";
			cout << code_s << endl;
			if(e1){
				code_s = "addi $sp, $sp, -4";
				cout << code_s << endl;
			...
			swap_register_stack(e1);
			
			expr2->generate_code(e2);
			
			swap_register_stack(e1);
			
			if(e1){
				code_s = "lwc1 $f" + to_string(freg) + ", 0($sp)";
				cout << code_s << endl;
				code_s = "addi $sp, $sp, 4";
				cout << code_s << endl;
			}
			code_s = "lw $" + to_string(reg) + ", 0($sp)";
			cout << code_s << endl;
			code_s = "addi $sp, $sp, 4";
			cout << code_s << endl;
		}
