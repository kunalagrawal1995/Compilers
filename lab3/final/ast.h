...
public:
	...
	int f_label; // min number of float label required
...
private:
	string fnname;
	EXP_GROUP *exp;		// parameters
public:
	int calculate_label(int parent, int is_left);  
	FUNC(string name, EXP_GROUP *expr);
	// var_type* get_type();
	void generate_code(int n);
	void print();
};
...
// print the commands, second implementation is for float, no imm in them
void output_command(string s, int dest, int r1, int r2, int imm);
void output_command(string s, int dest, int r1, int r2);

int generate_code_fn_init(string fnname);               //generates initialization  code of function (implemented as an independent function as there is no ExpAst for function definitions.)

//code geerated at end of function
void generate_code_fn_end(int local_stack_size);

int allocate_local_stack();

string generate_label(int n);

// max_parameter_offset for the function;
int get_max_param_offset(SymTab * fn_table);
